...
public:
    ...
    template<typename ViewHandler>
    void beforeController(ViewHandler &&viewHandler)
    {
        using ViewTraits = QHttpServerBeforeControllerViewTraits<ViewHandler>;
        static_assert(ViewTraits::Arguments::StaticAssert,
                      "ViewHandler arguments are in the wrong order or not supported");
        beforeControllerHelper<ViewTraits, ViewHandler>(std::move(viewHandler));
    }

    template<typename ViewHandler>
    void afterController(ViewHandler &&viewHandler)
    {
        using ViewTraits = QHttpServerBeforeControllerViewTraits<ViewHandler>;
        static_assert(ViewTraits::Arguments::StaticAssert,
                      "ViewHandler arguments are in the wrong order or not supported");
    ...
    template<typename ViewHandler>
    void AfterSendResponse(ViewHandler &&viewHandler)
    {
        using ViewTraits = QHttpServerBeforeControllerViewTraits<ViewHandler>;
        static_assert(ViewTraits::Arguments::StaticAssert,
                      "ViewHandler arguments are in the wrong order or not supported");
...
private:
    ...
    using BeforeControllerHandler =
        std::function<bool(bool &handled,QHttpServerResponder &responder,
                      const QHttpServerRequest &request)>;
    ...
    template<typename ViewTraits, typename ViewHandler>
    void beforeControllerHelper(ViewHandler &&viewHandler) {
        auto handler = [viewHandler](bool &handled,
                                     QHttpServerResponder &resp,
                                     const QHttpServerRequest &request) {
            if constexpr (ViewTraits::Arguments::Last::IsRequest::Value) {
                if constexpr (ViewTraits::Arguments::Count == 3)
                    return std::move(viewHandler(handled,resp, request));
                else
                    static_assert(dependent_false_v<ViewTraits>);
            } else if constexpr (ViewTraits::Arguments::Last::IsResponse::Value) {
                if constexpr (ViewTraits::Arguments::Count == 2)
                    return std::move(viewHandler(handled,resp));
                else if constexpr (ViewTraits::Arguments::Count == 3)
                    return std::move(viewHandler(handled,request, resp));
                else
                    static_assert(dependent_false_v<ViewTraits>);
            } else {
                static_assert(dependent_false_v<ViewTraits>);
        ...
        beforeControllerImpl(std::move(handler));
    }

    void beforeControllerImpl(BeforeControllerHandler beforeControllerHandler);
