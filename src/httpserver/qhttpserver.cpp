...
void QHttpServer::beforeControllerImpl(BeforeControllerHandler beforeControllerHandler)
{
    Q_D(QHttpServer);
    d->beforeControllerHandlers.push_back(std::move(beforeControllerHandler));
}
/*!
    \internal
*/
...
bool QHttpServer::handleRequest(const QHttpServerRequest &request, QHttpServerResponder &responder)
{
    ...
    for (auto beforeControllerHandler : d->beforeControllerHandlers){
        bool handled = false;
        if(!beforeControllerHandler(std::ref(handled),responder,request)){
            if(handled){
                return true;
            }
            return false;
